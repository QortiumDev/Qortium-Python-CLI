...
def _print_chat_command_help() -> None:
    ...
    print("/quit  /exit  /stop  /0   Leave chat.")
...
def _select_chat_thread_by_sender(
    ...
    *,
    ...
    sender_page = 0
    while True:
        sender_offset = sender_page * _PAGE_SIZE
        page_groups = user_groups[sender_offset : sender_offset + _PAGE_SIZE]
        has_next_senders = sender_offset + _PAGE_SIZE < len(user_groups)

        print_section(sender_section_title)
        if sender_page > 0:
            print(f"  (page {sender_page + 1})")
        for slot, (_, label, threads) in enumerate(page_groups, start=1):
            snippet = _chat_message_snippet(threads[0])
            if len(snippet) > 50:
                snippet = snippet[:47] + "..."
            print_option(str(slot), f"{label}  —  {snippet}")
        if has_next_senders:
            print_option("9", "Next page")
        print_option("0", "Back")

        choice = read_menu_choice(sender_prompt).strip()
        if choice == "0":
            return None
        if choice == "9" and has_next_senders:
            sender_page += 1
            continue

        try:
            slot = int(choice) - 1
            if slot < 0 or slot >= len(page_groups):
                raise IndexError
            _, label, sender_threads = page_groups[slot]
        except (ValueError, IndexError):
            warn("Unknown option.")
            continue

        # Message selection for the chosen sender
        msg_page = 0
        while True:
            msg_offset = msg_page * _PAGE_SIZE
            page_threads = sender_threads[msg_offset : msg_offset + _PAGE_SIZE]
            has_next_msgs = msg_offset + _PAGE_SIZE < len(sender_threads)

            print_section(f"{message_section_prefix} {label}")
            if msg_page > 0:
                print(f"  (page {msg_page + 1})")
            for slot, thread in enumerate(page_threads, start=1):
                print_option(str(slot), _format_replyable_chat_thread(thread))
            if has_next_msgs:
                print_option("9", "Next page")
            print_option("0", "Back")

            choice = read_menu_choice(message_prompt).strip()
            if choice == "0":
                break
            if choice == "9" and has_next_msgs:
                msg_page += 1
                continue
            try:
                slot = int(choice) - 1
                if slot < 0 or slot >= len(page_threads):
                    raise IndexError
                return page_threads[slot]
            except (ValueError, IndexError):
                warn("Unknown option.")
...
def run_chat_room(ctx: AppContext) -> None:
    ...
    while True:
        ...
        if stripped == "":
            continue
        if command in {"/quit", "/exit", "/stop", "/0"}:
            return
        if command in {"/help", "/?"}:
...
def build_tool_plugins() -> List[ToolPlugin]:
    from qortium_cli.tools.minting import tool_minting
    ...
    tools = [
        ...
        ToolPlugin("4", "Names", "Register, update, and manage names", tool_register_name),
    ]
    ...
    tools.extend([
        ...
        ToolPlugin("U", "Update", "Check for updates and install", tool_check_for_updates),
    ])
    return tools
